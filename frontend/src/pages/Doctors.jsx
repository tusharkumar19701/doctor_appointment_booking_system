  ...
  return (
    <div>
      ...
      <div className='flex flex-col sm:flex-row items-start gap-5 mt-5'>
        <button className={`py-1 px-3 rounded border text-sm transition-all sm:hidden ${showFilter ? "bg-[#5f6FFF] text-white": ""}`} onClick={()=>setShowFilter(prev=> !prev)}>Filters</button>
        ...
        <div className='w-full grid gap-4 gap-y-6' style={{gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))"}}>
          {
            filterDoctors.map((item,index)=> (
                item.available && <div onClick={()=>navigate(`/appointment/${item._id}`)} key={index} className="border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500 ">
